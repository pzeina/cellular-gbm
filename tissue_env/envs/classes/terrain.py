...
class GameMap:
    ...
    def __init__(self, width: int = 100, height: int = 100, cell_size: int = 6) -> None:
    ...
    def save_to_csv(self, path: Path) -> None:
        ...
        with path.open("w", newline="") as csvfile:
            ...
            for y in range(self.height):
                ...
                writer.writerow(row)  # print(f"Map saved to {path}")
        ...
        with path.open("r", newline="") as csvfile:
            ...
            for y, row in enumerate(reader):
                for x, cell_data in enumerate(row):
                    ...
                    game_map.altitude[y, x] = float(altitude_str)  # print(f"Map loaded from {path}")
    ...
    def get_visibility_map(self) -> np.ndarray:
        ...
        visibility_map = np.zeros((self.height, self.width))
        for y in range(self.height):
            for x in range(self.width):
                terrain: Terrain = self.terrain[y, x]
                if terrain:
